...
Const gistOwner = "jaredpohl"

Public Function uploadToDB()
    Dim ds As cDataSet, cb As cBrowser
    Set cb = New cBrowser
    Set ds = New cDataSet
    Dim url As String
    url = "http://requestb.in/146wqdv1"
   
    Dim email As String
    Dim token As String
    
    email = InputBox("Email:", "Inputbox")
    token = InputBox("Token", "Inputbox")
    
    Dim job As cJobject
    Set job = New cJobject
    
    'load company estimates as a json object
    ds.load ("Sheet1")
    ...
    With job
        ...
        .init Nothing
        'email
        With .add("email", email)
        End With
        'auth token
        With .add("auth_token", token)
        End With
        'add company estimates data
        With .append(ds.jObject)
        End With
    
    End With
    
    'push the data to the rest api on the database
    cb.httpPost url, job.stringify, True
